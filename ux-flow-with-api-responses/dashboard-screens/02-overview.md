...
All endpoints in this section follow a **single unified envelope**:
...
**Error envelope** (same shape for 400, 401, 403, 500):
...
> Preference cards-er monthly trend chart render korar jonno data return kore. YoY comparison inline series e embedded — frontend ke আলাদা calculate korte hobe na.
...
> Active subscriptions-er monthly analytics return kore — same shape as 2.2. YoY data inline series e embedded.
...
**Field Reference:** Same schema as 2.2. Additional fields:
...
|---|----------|:------:|:----:|:------:|-------|
| 2.1 | `/admin/growth-metrics` | GET | SUPER_ADMIN | ✅ Done | Summary stats — `changePct` always positive, use `direction` for sign |
| 2.2 | `/admin/preference-cards/monthly` | GET | SUPER_ADMIN | ✅ Done | Monthly trend — YoY inline per series item, accepts `year`/`compare_year`/`tz` params |
| 2.3 | `/admin/subscriptions/active/monthly` | GET | SUPER_ADMIN | ✅ Done | Same shape as 2.2 + `is_peak`/`is_slowest` boolean flags per item |
